...
public:
  ...
  [[eosio::action]] void nextround(eosio::name username);

  [[eosio::action]] void endgame(eosio::name username);
...
private:
  ...
  int ai_min_loss_strategy(const int ai_attack_point,
                           const int player_attack_point);

  int ai_points_tally_strategy(const int ai_attack_point,
                               const int player_attack_point);

  int ai_loss_prevention_strategy(const int8_t life_ai,
  ...
  int ai_choose_card(const game &game);

  void resolve_selected_cards(game & game_data);

  void update_game_status(user & user_record);
